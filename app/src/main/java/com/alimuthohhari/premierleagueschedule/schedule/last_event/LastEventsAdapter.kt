...
class LastEventsAdapter(private val items: List<ListEvents>, private val clickListener: (ListEvents) -> Unit) :
    RecyclerView.Adapter<LastEventsAdapter.ViewHolder>() {
    class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindItem(items: ListEvents, clickListener: (ListEvents) -> Unit) {
            ...
            var time = timeCalnder.toSimpleString(items.strTime!!)
            if (time.equals(null)) time = timeCalnder2.toSimpleString(items.strTime!!)
